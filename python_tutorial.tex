...
\end{itemize}
	...
	\begin{lstlisting}[caption={Métodos de lista}]
meus_personagens = ["Goku", "Vegeta", "Kuririn", "Freeza"]

# posso iterar pela lista
for personagem in meus_personagens:
print(personagem)

# posso adicionar mais um personagem ao fim da lista
meus_personagens.append("Piccolo")

# posso adicionar numeros, booleanas ou quaisquer variaveis de tipos diferentes
# nota: isso pode nao ser desejavel - no entanto, eh o que acontece.
meus_personagens.append(52)
meus_personagens.append(True)

for personagem in meus_personagens:
print(personagem)

# mas e se eu quiser remover os que nao sao personagens do fim da lista?
meus_personagens.pop()
meus_personagens.pop()
# pronto, feito.

# e para adicionar elementos ao meio da lista?
meus_personagens.insert(2, "Android 17")

# lembre-se: listas sao indexadas comecando do 0.

# o metodo enumerate serve para obter o indice junto com o valor, veja:
for i, personagem in enumerate(meus_personagens):
print("o personagem", i, "se chama", personagem)

# posso acessar um personagem qualquer da lista usando colchetes
# lembre-se: o indice comeca em 0, entao isso acessa o primeiro personagem
print(meus_personagens[0])

# fim do exemplo :)\end{lstlisting}

\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=1.8cm, every node/.style={draw, minimum width=2cm, minimum height=1cm}, >=stealth]
		
		% Elementos da lista
		\node (el0) {Goku};
		\node[right of=el0] (el1) {Vegeta};
		\node[right of=el1] (el2) {Kuririn};
		\node[right of=el2] (el3) {Freeza};
		
		% Índices positivos abaixo
		\node[below=0.2cm of el0] {\scriptsize \texttt{[0]}};
		\node[below=0.2cm of el1] {\scriptsize \texttt{[1]}};
		\node[below=0.2cm of el2] {\scriptsize \texttt{[2]}};
		\node[below=0.2cm of el3] {\scriptsize \texttt{[3]}};
		
		% Índices negativos acima
		\node[above=0.2cm of el0] {\scriptsize \texttt{[-4]}};
		\node[above=0.2cm of el1] {\scriptsize \texttt{[-3]}};
		\node[above=0.2cm of el2] {\scriptsize \texttt{[-2]}};
		\node[above=0.2cm of el3] {\scriptsize \texttt{[-1]}};
		
		% Setas de loop (para sugerir circularidade)
		\draw[->, thick] (el3.east) to[out=0, in=270] ++(0.5, -0.7) to[out=90, in=0] (el0.east);
		
	\end{tikzpicture}
	\caption{Representação de uma lista com indexação positiva e negativa.}
\end{figure}
	...
	% ====================================================
	% AINDA NÃO ACABOU TUDO POR AQUI
	% AINDA NÃO ACABOU TUDO POR AQUI
	% AINDA NÃO ACABOU TUDO POR AQUI
	% AINDA NÃO ACABOU TUDO POR AQUI
	% AINDA NÃO ACABOU TUDO POR AQUI
